            ...
            <DropdownMenu>
              <DropdownMenuTrigger asChild>
                <Button variant="ghost" className="hidden md:flex items-center gap-2">
                  ...
                  <span className="hidden lg:inline-block text-sm font-medium">{displayName}</span>
              ...
              <DropdownMenuContent align="end" className="w-56">
                <DropdownMenuLabel>
                  <div className="flex flex-col space-y-2">
                    ...
                    <span className={`inline-flex items-center w-fit px-2 py-1 rounded-full text-xs font-medium ${
                      displayRole === "Admin"
                ...
                {navLinks.map((link) => {
                  const getIcon = () => {
                    if (link.label === "Dashboard") return <Home className="mr-2 h-4 w-4" />;
                    if (link.label === "Certifications") return <Award className="mr-2 h-4 w-4" />;
                    if (link.label === "Users") return <Users className="mr-2 h-4 w-4" />;
                    return null;
                  };
                  return (
      ...
      {mobileMenuOpen && (
        <div className="border-t md:hidden">
          <div className="container mx-auto space-y-1 px-4 py-4">
            ...
            {navLinks.map((link) => {
              const getIcon = () => {
                if (link.label === "Dashboard") return <Home className="h-4 w-4" />;
                if (link.label === "Certifications") return <Award className="h-4 w-4" />;
                if (link.label === "Users") return <Users className="h-4 w-4" />;
                return null;
              };
              return (
                <Link
                  ...
                  className="flex items-center gap-2 rounded-lg px-3 py-2 text-sm font-medium transition-colors text-foreground hover:bg-muted"
                >
                  {getIcon()}
                  {link.label}
                </Link>
